...
function assertFundingRelayCoverage(payload) {
  const status = payload?.relayStatus;
  if (!status?.configured) return;

  const requested = Array.isArray(status.requested)
    ? status.requested.map(textValue).filter(Boolean)
    : [];
  const received = new Set(Array.isArray(status.received)
    ? status.received.map(textValue)
    : []);
  const failureCodes = status.failures && typeof status.failures === "object"
    ? Object.fromEntries(Object.entries(status.failures).map(([name, code]) => [
      textValue(name),
      /^[a-z0-9_-]+$/i.test(textValue(code)) ? textValue(code) : "network"
    ]))
    : {};
  const unavailable = requested.filter((name) => !received.has(name) || failureCodes[name]);
  if (unavailable.length > 0) {
    const details = unavailable.map((name) => failureCodes[name]
      ? `${name} (${failureCodes[name]})`
      : name);
    throw new Error(`Funding relay incomplete for: ${details.join(", ")}`);
  }
}
...
async function postFundingSheetSnapshot(config, snapshot, fetchImpl = fetch, options = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      ...
      if (response.ok || !isRetryableSheetStatus(response.status) || attempt === maxAttempts) break;
    } catch (error) {
