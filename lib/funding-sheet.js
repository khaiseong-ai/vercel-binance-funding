...
function assertFundingRelayCoverage(payload) {
  ...
  const received = new Set(Array.isArray(status.received)
    ...
    : []);
  const failures = new Set(Array.isArray(status.failures)
    ? status.failures.map(textValue)
    : []);
  const unavailable = requested.filter((name) => !received.has(name) || failures.has(name));
  if (unavailable.length > 0) {
    throw new Error(`Funding relay incomplete for: ${unavailable.join(", ")}`);
  }
}
